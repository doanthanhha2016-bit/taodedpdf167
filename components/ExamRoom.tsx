...
// ✅ Loại bỏ các field undefined trước khi lưu Firestore
// Firestore không chấp nhận undefined — phải là null hoặc bỏ qua field
function sanitizeStudent(s: StudentInfo): Record<string, unknown> {
  const result: Record<string, unknown> = { id: s.id, name: s.name };
  if (s.email     !== undefined) result.email     = s.email;
  if (s.avatar    !== undefined) result.avatar    = s.avatar;
  if (s.className !== undefined) result.className = s.className;
  if (s.classId   !== undefined) result.classId   = s.classId;
  if (s.studentId !== undefined) result.studentId = s.studentId;
  return result;
}
    ...
    const loadExam = async () => {
      try {
        ...
        if (examData) {
          ...
          // ✅ FIX DEDUP: credential students dùng stable "student_username" làm student.id
          // vì anonymous UID thay đổi mỗi phiên → không dedup được
          // sanitizeStudent: loại bỏ field undefined → Firestore không lỗi
          const stableId = student.studentId || uid;
          const fixedStudent = sanitizeStudent({ ...student, id: stableId });
