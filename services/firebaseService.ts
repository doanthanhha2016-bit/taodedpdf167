...
export const getStudentSubmission = async (
  roomId: string,
  studentId: string,
  studentName?: string   // fallback cho submissions cũ
): Promise<Submission | null> => {
  // Query 1: theo student.id (stable username hoặc auth.uid)
  const q1 = query(
  ...
  if (!s1.empty) return parseSubmissionData(s1.docs[0].id, s1.docs[0].data());

  // Query 2: theo student.studentId (credential student field)
  const q2 = query(
    collection(db, 'submissions'),
    where('roomId', '==', roomId),
  ...
  if (!s2.empty) return parseSubmissionData(s2.docs[0].id, s2.docs[0].data());

  // Query 3: theo student.name — fallback cho submissions cũ không có studentId
  // (submissions tạo trước khi có fix, lưu với anonymous UID cũ)
  if (studentName) {
    const q3 = query(
      collection(db, 'submissions'),
      where('roomId', '==', roomId),
    ...
    if (!s3.empty) {
      // Lấy bài nộp mới nhất nếu có nhiều
      const all = s3.docs.map(d => parseSubmissionData(d.id, d.data()));
      all.sort((a, b) => (b.submittedAt?.getTime() || b.startedAt?.getTime() || 0)
                       - (a.submittedAt?.getTime() || a.startedAt?.getTime() || 0));
      return all[0];
    }
  }
...
export const getSubmissionsByStudent = async (studentId: string): Promise<Submission[]> => {
  // Query theo student.id (Firebase UID)
  const q1 = query(collection(db, 'submissions'), where('student.id', '==', studentId));
  const snap1 = await getDocs(q1);

  // Fallback: query theo student.studentId (stable username cho credential students)
  const q2 = query(collection(db, 'submissions'), where('student.studentId', '==', studentId));
  const snap2 = await getDocs(q2);

  // Gộp kết quả, loại trùng theo submissionId
  ...
  return allDocs
    .map(d => parseSubmissionData(d.id, d.data()))
...
export const getExam = async (examId: string): Promise<Exam | null> => {
  ...
  return {
    ...
    // ✅ Drive fields cho Đề thi
    pdfDriveUrl: data.pdfDriveUrl,
    pdfDriveFileId: data.pdfDriveFileId,

    // ✅ MỚI: Thêm 3 dòng này để lấy đầy đủ dữ liệu Lời giải
    solutionPdfDriveUrl: data.solutionPdfDriveUrl,
    solutionPdfDriveFileId: data.solutionPdfDriveFileId,
    questionSolutions: data.questionSolutions,

    // ✅ FIX: Overlay drag-drop — thiếu 2 dòng này khiến vị trí kéo thả
    // bị mất khi đọc lại từ Firestore (học sinh thấy vị trí auto-detect)
    overlayMode: data.overlayMode,
    pdfOverlayControls: data.pdfOverlayControls,

    // Backward compat (cho các đề thi cũ)
...
export const getExamsByTeacher = async (teacherId: string): Promise<Exam[]> => {
  ...
  const exams = snapshot.docs.map((docSnap) => {
    ...
    return {
      ...
      // ✅ MỚI: Thêm 3 dòng này để lấy dữ liệu Lời giải
      solutionPdfDriveUrl: data.solutionPdfDriveUrl,
      solutionPdfDriveFileId: data.solutionPdfDriveFileId,
      questionSolutions: data.questionSolutions,

      // ✅ FIX: Overlay drag-drop
      overlayMode: data.overlayMode,
      pdfOverlayControls: data.pdfOverlayControls,
